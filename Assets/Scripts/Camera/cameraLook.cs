...
public class cameraLook : MonoBehaviour
{
    ...
    public float horizAngle = 0;

    // Vertical angle of the camera (x-axis)
    public float vertAngle = 0;
    ...
    void LateUpdate()
    {
        if(!pauseMenu.paused)
        {
            ...
            if (SettingsManager.horizSensitivity != horizSensitivity)
            {
                horizSensitivity = SettingsManager.horizSensitivity;
            }
            if (SettingsManager.vertSensitivity != vertSensitivity)
            {
